...
import com.example.eden.dict.utils.JSContentHelper;
import com.example.eden.dict.utils.NetOperationHelper;
import com.example.eden.dict.utils.XMLParseHelper;
...
public class Dict {
    ...
    public Dict(Context context,String tableName){
        ...
        dbHelper=new DatabaseHelper(context, tableName);     //这里要用到前面的DataBaseHelper类，在Dict的构造方法中实例化该类，
...
//                "interpret text, sentorig text, senttrans text)");
    ...
    public WordValue getWordFromInternet(String searchedWord){
        ...
        /*if(array[0]>'z')           //是中文，或其他语言的的简略判断
            tempWord="_"+URLEncoder.encode(tempWord);
*/
        InputStream in=null;
        String str=null;
        try{
            String tempUrl= NetOperationHelper.sURL1+tempWord+ NetOperationHelper.sURL2;
            in= NetOperationHelper.getInputStreamByUrl(tempUrl);
            if(in!=null){
                new FileHelper().saveInputStreamToFile(in, "", "gfdgf.txt");
                XMLParseHelper xmlParser=new XMLParseHelper();
                InputStreamReader reader=new InputStreamReader(in,"utf-8");
                JSContentHelper contentHandler=new JSContentHelper();
